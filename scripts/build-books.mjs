...
const ASK_CSS = `
  .af-hero{display:grid;grid-template-columns:235px minmax(390px,1fr) minmax(320px,380px);gap:clamp(26px,2.8vw,46px);align-items:start;padding:30px 0 12px}
  ...
  /* Mobile: the header was a tall box shoving the questions down. Shrink it so
     the answers sit high on the page. */
  @media(max-width:720px){
    .af-ask-head{padding:14px 18px}
    .af-ask-head>span{font-size:.6rem}
    .af-ask-head h2{font-size:1.45rem;margin-top:5px;line-height:1.08}
  }
  ...
  .af-snip{grid-column:1/-1;margin:0 0 16px;padding:12px 22px 4px}
  .af-snip p{margin:0 0 10px;color:var(--muted);font-size:.92rem;line-height:1.55}
  .af-snip .af-snip-spoiler{color:var(--cream);font-family:'Fraunces',serif;font-weight:500;font-size:1.15rem;line-height:1.3}
  ...
  @media(prefers-reduced-motion:reduce){.af-layer,.af-drawer,.af-sheet{animation:none}}

  /* intentional empty-state copy — never a blank block */
  .af-empty-note{margin-top:12px;color:var(--muted);font-size:.9rem;line-height:1.55;max-width:60ch}

  /* ── Mobile: persistent bottom action dock (book pages only) ──────────────
     On mobile the top action row and the "Finished it?" review block are
     hidden (they'd duplicate the dock) and their functions move here, within
     thumb's reach. The dock is fixed only at the mobile breakpoint; on desktop
     it never renders or reserves space. */
  .af-dock{display:none}
  @media(max-width:720px){
    /* the duplicated top-of-page shelf + review controls */
    .af-actions, .af-rate, .shelf-control{display:none}
    ...
    .af-dock .af-dock-share{background:var(--panel);color:var(--cream);border:1px solid var(--line);min-width:48px;padding:.7em .8em;font-size:1.1rem}
    .af-dock-note{position:fixed;left:0;right:0;bottom:calc(56px + env(safe-area-inset-bottom));z-index:40;text-align:center;font-size:.72rem;color:var(--muted);pointer-events:none;opacity:0;transition:opacity .2s}
    .af-dock-note.show{opacity:1}
    .af-dock-note a{color:var(--amber);pointer-events:auto}
  ...
  /* ── Review bottom sheet (mobile) — the relocated "Finished it?" spice rating.
     Slides up from the bottom; focus-trapped, Escape closes, backdrop dismisses. */
  body.af-sheet-open{overflow:hidden}
  .af-sheet-layer{position:fixed;inset:0;z-index:90;display:flex;align-items:flex-end;background:rgba(4,2,3,.72);backdrop-filter:blur(4px);animation:af-fade .18s ease-out}
  .af-sheet{width:100%;max-height:80vh;overflow-y:auto;background:var(--panel);border-top:1px solid var(--line);border-radius:18px 18px 0 0;padding:20px 20px calc(22px + env(safe-area-inset-bottom));box-shadow:0 -20px 50px rgba(0,0,0,.5);animation:af-up .26s ease-out}
  .af-sheet-grip{width:40px;height:4px;border-radius:99px;background:var(--line);margin:0 auto 14px}
  .af-sheet-head{display:flex;justify-content:space-between;align-items:flex-start;gap:16px}
  .af-sheet-head h2{font-family:'Fraunces',serif;font-weight:600;font-size:1.3rem}
  .af-sheet-head>div>span{color:var(--amber);font-size:.62rem;font-weight:800;letter-spacing:.14em;text-transform:uppercase}
  .af-sheet-head button{flex:0 0 38px;height:38px;background:none;border:1px solid var(--line);border-radius:50%;color:var(--cream);font-size:1.3rem;line-height:1;cursor:pointer}
  .af-sheet-head button:hover{border-color:var(--rose)}
  /* The review sheet reuses the real .af-rate block (moved in on open), so the
     rating logic operates on the same nodes with no duplication. These rules
     un-hide it inside the sheet and strip its box chrome (the sheet supplies
     its own) and its redundant "Finished it?" label (the sheet head has it). */
  .af-sheet .af-rate{display:block;border:0;background:transparent;padding:0;margin:0}
  .af-sheet .af-rate>span{display:none}
  .af-sheet .af-stars{margin-top:4px;justify-content:space-between;gap:8px}
  .af-sheet .af-stars button{font-size:2.1rem;padding:4px}
  .af-sheet .af-rate-note{margin-top:14px;font-size:.9rem}
...
function afFacts(b){
  ...
  // Split "First person · single" into a value + sub so a fact cell never wraps
...
<!-- Persistent mobile action dock — CSS hides it entirely above 720px, so it
     never renders or reserves space on desktop. Book/blurb pages only: it lives
...
<p class="af-dock-note" id="afDockNote" role="status" aria-live="polite"></p>
...
<script>
  ...
  (function(){
    ...
    var LABEL = { want:'TBR', reading:'Currently Reading', read:'Read', dnf:'DNF' };
    ...
    async function shelve(status){
      ...
      if (status==='read') payload.finished_at = new Date().toISOString();   // powers dashboard pace/"read this year"
    ...
    // Brief transient feedback above the dock (the sidebar shelf note is hidden
    // on mobile). Auto-clears so it never sits over the footer.
    var dockNoteTimer = null;
    function dockToast(html){
      if (!dockNote) return;
      dockNote.innerHTML = html; dockNote.classList.add('show');
      clearTimeout(dockNoteTimer); dockNoteTimer = setTimeout(function(){ dockNote.classList.remove('show'); }, 2600);
    }
    function toggleWant(){
    ...
    async function doShare(){
      ...
      try {
        ...
        else { await navigator.clipboard.writeText(url); if (afShare) afShare.textContent = '✓ Link copied'; dockToast('Link copied'); setTimeout(function(){ if (afShare) afShare.textContent = '↗ Share'; }, 1800); }
        if (window.SH && SH.track) SH.track('share', { where: 'book-page' });
      } catch(e){ /* user dismissed the share sheet — nothing to report */ }
    }
    if (afShare) afShare.addEventListener('click', doShare);
    if (dockShare) dockShare.addEventListener('click', doShare);
    ...
    (async function init(){
      ...
      try {
        ...
        if (user){
          var r = await sb.from('shelf').select('status').eq('user_id', user.id).eq('book_key', BOOK.key).maybeSingle();
...
<script>
  // ── Review bottom sheet (mobile dock) ────────────────────────────────────
  // Opening relocates the real .af-rate block (#af-review) into the sheet so
  // the existing spice-rating logic keeps working on the same nodes — one
  // rating control, no duplicate state. Accessible dialog: focus-trapped,
  // Escape and backdrop close, focus restored, body scroll locked. On close
  // the rating block is returned to its original position.
  (function(){
    var trigger = document.getElementById('afDockReview');
    var rate = document.getElementById('af-review');
    if (!trigger || !rate) return;
    var homeParent = rate.parentNode, homeNext = rate.nextSibling;   // where to put it back
    var layer = null, lastFocus = null;

    function close(){
      if (!layer) return;
      if (homeNext) homeParent.insertBefore(rate, homeNext); else homeParent.appendChild(rate);
      layer.remove(); layer = null;
      document.body.classList.remove('af-sheet-open');
      if (lastFocus && lastFocus.focus) lastFocus.focus();
    }
    function open(){
      lastFocus = document.activeElement;
      layer = document.createElement('div');
      layer.className = 'af-sheet-layer';
      layer.innerHTML =
        '<aside class="af-sheet" role="dialog" aria-modal="true" aria-labelledby="afSheetTitle">'
        + '<div class="af-sheet-grip" aria-hidden="true"></div>'
        + '<div class="af-sheet-head"><div><span>Finished it?</span>'
        + '<h2 id="afSheetTitle">Rate the spice</h2></div>'
        + '<button type="button" class="af-sheet-close" aria-label="Close">×</button></div>'
        + '<div class="af-sheet-body"></div></aside>';
      layer.querySelector('.af-sheet-body').appendChild(rate);   // move the live rating in
      document.body.appendChild(layer);
      document.body.classList.add('af-sheet-open');
      layer.querySelector('.af-sheet-close').focus();
      layer.addEventListener('mousedown', function(e){ if (e.target === layer) close(); });
      layer.querySelector('.af-sheet-close').addEventListener('click', close);
      if (window.SH && SH.track) SH.track('review-open', { where: 'book-page-dock' });
    }
  ...
  (function(){
    ...
    (async function init(){
      ...
      try {
        ...
        user = u && u.data ? u.data.user : null;
        if (user){
          var r = await sb.from('book_tags').select('spice').eq('user_id', user.id).eq('book_key', KEY).maybeSingle();
  ...
  (function(){
    ...
    nav.querySelectorAll('a').forEach(function(a){
      a.addEventListener('click', function(e){
        ...
        if (a.getAttribute('data-spoiler') === '1'){
          // The tap IS the opt-in: reveal the ending inline right here, and
          // unlock the matching section at the bottom too — no second click.
          var vEl = sec ? sec.querySelector('.af-ending-verdict') : null;
          snip.innerHTML = '<p class="af-snip-spoiler">' + esc(vEl ? vEl.textContent.trim() : 'Not recorded.') + '</p>';
          if (sec){
            var body = sec.querySelector('.af-ending-body'), btn = sec.querySelector('.af-reveal');
            if (body) body.hidden = false;
            if (btn){ btn.setAttribute('aria-expanded', 'true'); btn.textContent = '× Hide the ending'; }
          }
          var em = a.querySelector('.af-qa em'); if (em) em.textContent = 'revealed';
          if (window.SH && SH.track) SH.track('ending-reveal', { where: 'ask-list' });
        } else {
        ...
        var more = snip.querySelector('.af-snip-more');
        if (more) more.addEventListener('click', function(){ gotoSection(sec); });
