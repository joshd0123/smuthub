...
<style>
  ...
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  /* Sticky left-edge category navigator — shown on every screen size now
     (mobile readers need the quick jump between categories most of all). */
  ...
  .sh-rail-panel{position:absolute;left:100%;top:50%;transform:translate(-12px,-50%);background:var(--ink-2);border:1px solid var(--line);border-radius:14px;padding:14px;width:min(280px,calc(100vw - 54px));max-height:80vh;overflow-y:auto;opacity:0;pointer-events:none;transition:opacity .15s,transform .15s;box-shadow:0 22px 50px rgba(0,0,0,.55);margin-left:8px}
...
const RAIL_SCRIPT = `<script>(function(){
  ...
  // Close the panel the instant any category is chosen. On touch the panel
  // overlays the content, so leaving it open after a tap forces the reader to
  // "click off" (and risks tapping a term behind it). Fires on every page —
  // for a navigating link it closes then follows; for the same-page anchor
  // links below it just doubles the setOpen(false) they already do.
